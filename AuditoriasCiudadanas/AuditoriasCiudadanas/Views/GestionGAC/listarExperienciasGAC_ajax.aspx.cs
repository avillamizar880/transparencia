...
{
    public partial class listarExperienciasGAC_ajax : App_Code.PageSession
    {
            ...
            if (HttpContext.Current.Request.HttpMethod == "POST")
            {
                ...
                if (opcion.ToUpper().Equals("OBT"))
                {
                    if (id_recurso_aux > 0)
                    {
                        ...
                        outTxt = datos_func.obtExperienciaGacById(id_recurso_aux);
                    }
                }
                else if (opcion.ToUpper().Equals("LIST"))
                {
                    AuditoriasCiudadanas.Controllers.GestionGruposController datos_func = new Controllers.GestionGruposController();
                    objReturn = datos_func.obtExperienciasGAC(page_aux);
                ...
                {
                    AuditoriasCiudadanas.Controllers.GestionGruposController datos_func = new Controllers.GestionGruposController();
                    outTxt = datos_func.aprobarExperienciasGAC(id_recurso_aux, id_usuario_aux);
                }
