...
namespace AuditoriasCiudadanas.Models
{
    public class clsValoracion
    {
        ...
        public static List<DataTable> obtCuestionarioPosterior(string bpin_proyecto)
        {
            List<DataTable> Data = new List<DataTable>();
            List<PaParams> parametros = new List<PaParams>();
            parametros.Add(new PaParams("@CodigoBPIN", SqlDbType.VarChar, bpin_proyecto, ParameterDirection.Input, 15));
            Data = DbManagement.getDatos("dbo.pa_obt_cuestionario_posterior", CommandType.StoredProcedure, cadTransparencia, parametros);
            return Data;

        }

        public static string eliminarPregunta(int id_pregunta) {
            string outTxt = "";
            string cod_error = "-1";
            string mensaje_error = "@ERROR";
            List<DataTable> Data = new List<DataTable>();
            ...
            parametros.Add(new PaParams("@cod_error", SqlDbType.Int, cod_error, ParameterDirection.Output));
            parametros.Add(new PaParams("@mensaje_error", SqlDbType.VarChar, mensaje_error, ParameterDirection.Output));
            Data = DbManagement.getDatos("dbo.pa_del_pregunta", CommandType.StoredProcedure, cadTransparencia, parametros);
            if (Data.Count > 1)
            {
                if (Data[1].Rows.Count > 0)
                {
