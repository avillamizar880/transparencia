...
namespace AuditoriasCiudadanas.Models
{
  public class clsCorreosProgramados
  {
    ...
    /// <summary>
    /// Notificacion correo para tareas a vencer
    /// </summary>
    /// <param name="fechaValidacion"></param>
    /// <returns></returns>
    public static String GenerarCorreoTareasVencer(DateTime fechaValidacion)
    {
      string outTxt = "";
      string mensaje = "";
      EnvioCorreosController datos_func = new EnvioCorreosController();
      //string url_local = datos_func.obtUrlLocal();
      DataTable dtConfig = clsPlanTrabajo.ObtenerTareasVencer(fechaValidacion);
      DataTable dtDatosCorreo= clsEnvioCorreos.obtCuentaCorreo(1)[0];
      //string link = url_local + "/login?params=" + HttpUtility.UrlEncode(App_Code.SafeParams.encode("GestionGAC/AprobacionBuenasPracticas|dvPrincipal|"));
      if (dtConfig.Rows.Count > 0)
      {
        foreach (DataRow drTareaVencer in dtConfig.Rows)
        {
          try
          {
            var tipoTarea= drTareaVencer.ItemArray[0] != DBNull.Value ? drTareaVencer.ItemArray[0].ToString() : string.Empty;
            var fechaDefinida = drTareaVencer.ItemArray[1] != DBNull.Value ? drTareaVencer.ItemArray[1].ToString() : string.Empty;
            var fechaVencimiento = drTareaVencer.ItemArray[1] != DBNull.Value ? drTareaVencer.ItemArray[1].ToString() : string.Empty;
            var email = drTareaVencer.ItemArray[3] != DBNull.Value ? drTareaVencer.ItemArray[3].ToString() : string.Empty;
            mensaje += "<html>";
            mensaje += "<head>";
            mensaje += "<title>Auditorias Ciudadanas - Notificaciones</title>";
            mensaje += "<style>p {color:#fff;font-family:Tahoma, Geneva, sans-serif;font-size:16px;} h1 {font-family:Tahoma, Geneva, sans-serif;font-size:2em;}";
            ...
            mensaje += "<body style=\"font-family:Tahoma, Geneva, sans-serif\">";
            mensaje += "<table  style=\"color:#fff;background-color:#2AAAE2; width:600px;  margin:0 auto; padding:25px 0px; color:#FFFFFF;\">";
            mensaje += "<tr><td style=\"width:200px\"><img src=\"" + "/Content/img/icon_experiencias.gif\" width=\"100%\" alt=\"Buenas prácticas\"/></td>";
            mensaje += "<td style=\"text-align:center\"><h1 style=\"margin: 15px\">Vencimiento de tarea</h1>";
            mensaje += "<p style=\"width:60%; margin:0 auto; text-align:center\">Tienes una tarea próxima a vencer, a continuación compartimos los detalles a tener en cuenta para esta tarea.\nTipo de tarea: "+ tipoTarea + "\nFecha Terminación tarea:" + fechaVencimiento +  "</p><br />";
            //mensaje += "<a href=\"" + link + "\" style=\"background-color:#8AC844; border-bottom:3px solid #278CB8; padding:5px 25px; color:#fff; font-weight:bold;\">";
            //mensaje += "VER</a>";
            mensaje += "</td></tr></table>";
            mensaje += "</body></html>";
            outTxt = App_Code.CorreoUtilidad.envCorreoNet(mensaje, email, null, null, "Vencimiento de Tareas", dtDatosCorreo);
          }
          catch (Exception ex)
          { 
          }
