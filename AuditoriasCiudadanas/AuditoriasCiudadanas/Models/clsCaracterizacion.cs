...
namespace AuditoriasCiudadanas.Models
{
  static public class clsCaracterizacionModels
  {
    ...
    public static DataTable ObtenerDatosEncuestaUsuario(int pagina, int usuarioId, string nombreMunicipio)
    {
    ...
    static public string IngresarEncuesta(int pagina, params object[] parametrosGuardar)
    {
      ...
      switch (pagina)
      {
        ...
        case 2:
          ...
          if (parametrosGuardar == null || parametrosGuardar.Length <= 13) return "-1";//Significa que los parámetros no son correctos
          ...
          var mecanismoHaParticipado = parametrosGuardar[3].ToString() != string.Empty ? parametrosGuardar[3].ToString() : parametrosGuardar[2].ToString();
          ...
          var recursosAlcaldia = parametrosGuardar[6].ToString();
          var auditoriasVisibles= parametrosGuardar[7].ToString();
          ...
          if (idDivipola == 0) return "-2"; //Significa que el municipio no existe en la base de datos
          if (!int.TryParse(parametrosGuardar[12].ToString(), out idUsuario)) return "-3";
          idEncuestaCaracterizacion = ObtenerIdEncuestaCaracterizacion(idUsuario, idDivipola);
    ...
    private static int ObtenerIdEncuestaCaracterizacion(int idUsuario, int idDivipola)
    {
      List<PaParams> parametros = new List<PaParams>();
      parametros.Add(new PaParams("@IdUsuario", SqlDbType.Int, idUsuario, ParameterDirection.Input));
      parametros.Add(new PaParams("@idDivipola", SqlDbType.VarChar, idDivipola.ToString(), ParameterDirection.Input, 15));
      DataTable dtRta = DbManagement.getDatosDataTable("pa_obt_idenccarac_idUsuidDiv", CommandType.StoredProcedure, cadTransparencia, parametros);
      int sal = 0;
