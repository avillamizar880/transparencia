...
namespace AuditoriasCiudadanas.Controllers
{
    ...
    public string EliminarGrupoAuditor(string parametrosEliminar)
    {
      return clsProyectos.EliminarGrupoAuditor(parametrosEliminar);
    }
    ...
    public string formato_miles(string cadena) {
        ...
        public string obtInfoProyecto(string id_proyecto,int id_usuario) {
            ...
            DataTable dtperfil = listaInfo[20];  //TRAER PERFIL
            ...
            string perfil = "";  //VARIABLE PARA SI ES ADMIN O TECNICO
                ...
                for (int i = 0; i <= dtGrupos.Rows.Count - 1; i++)
                {
                    ...
                    if (idGrupo != id_grupo_fila)
                    {
                        ...
                        if ((perfil == "2") && (tipo_rol != "3")) // no es tecnico dnp, ni interventor
                        {
                            ...
                            {
                                //buscar si el auditor está dentro del grupo
                                var results = dtGrupos.Rows.Cast<DataRow>()
                                              .FirstOrDefault(x => x.Field<int>("idUsuario") == id_usuario && x.Field<int>("idgrupo")==Convert.ToInt16(id_grupo_fila));
                                if(results!=null)
                               {
                        ...
                        if (perfil == "4")
                            tablaGrupos += "<div class=\"recuadroRedes\" data-toggle=\"tooltip\" title=\"Eliminar grupo auditor\"><a role=\"button\" onclick=\"EliminarGrupoAuditor(" + codigo_bpin + "," + id_grupo_fila + "," + id_usuario + ")\"  class=\"fr\" ><img src = \"../../Content/img/delgroup.png\"/></a>";
       ...
       return outTxt;
        ...
        public string obtGestionProyecto(string bpin_proyecto, int id_grupo, int id_usuario)
        {
            ...
            else if ((String.IsNullOrEmpty(ActaAudSeguimiento)) && (!String.IsNullOrEmpty(auditor)) && (yaPasoAudSeguimiento == "1")) //No hay acta, es auditor y ya ha pasado fecha de Seguimiento
            {
                ...
                {
                    AudienciaSeguimiento += "<div class=\"col-sm-5 botonGestion\"><a onclick =\"javascript:InsRegistroCompromisos(" + "\\'" + idAudSeguimiento + "\\'" + ");\"  role=\"button\" class=\"btn btn-default\"><span class=\"glyphicon glyphicon-file\"></span>Registrar Compromisos</a></div>";
                }
                else
                {
            ...
            else if ((yaPasoAudSeguimiento == "1") && (yaPasoAudCierre == "0")) //No ha pasado fecha de cierre
            {
                ...
                if (!String.IsNullOrEmpty(auditor)) //Es auditor
                {
                    InformeProcesoCierre += "<div class=\"col-sm-5\"><a onclick=\"javascript:informeproceso(" + "\\'" + bpin_proyecto + "\\'" + "," + "\\'" + id_usuario + "\\'" + ",2,1," + "\\'" + idAudSeguimiento + "\\'," + id_grupo + ");\" role=\"button\" class=\"btn btn-default\"><span class=\"glyphicon  glyphicon-file\"></span> Diligenciar Informe</a></div>";
                }
                else  //no es auditor
                {
                    InformeProcesoCierre += "<div class=\"col-sm-5\"><a onclick=\"javascript:informeproceso(" + "\\'" + bpin_proyecto + "\\'" + "," + "\\'" + id_usuario + "\\'" + ",2,2," + "\\'" + idAudSeguimiento + "\\'," + id_grupo + ");\" role=\"button\" class=\"btn btn-default\"><span class=\"glyphicon  glyphicon-eye-open\"></span> Ver Informe</a></div>";
                }
            }
            else if (yaPasoAudCierre == "1") //ya paso audiencia de cierre
            {
